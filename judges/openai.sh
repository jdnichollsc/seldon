#!/usr/bin/env bash
# Seldon judge-runner for OpenAI Chat Completions API (via curl)
...
# --- Build request ---
system_prompt="You are an independent plan reviewer. Evaluate the plan against the codebase. Return JSON matching the provided schema. Focus: $focus. $focus_inst"
...
# Extract content
content="$(echo "$response" | python3 -c 'import json,sys; r=json.load(sys.stdin); print(r["choices"][0]["message"]["content"])' 2>/dev/null)" || {
